...
| **搜索 / 筛选** | ✅ 首页顶部搜索图标（激活切换 search-active.png），支持备注/分类关键词（大小写不敏感）、收入/支出/全部类型 chips、金额区间（元）、日期区间筛选；结果实时更新并显示匹配条数 |
| **云同步** | 使用微信云开发 CloudDB（`db.collection('records')`），本地 Storage 作缓存层，保证离线可用；登录即同步，换手机不丢数据 |
| **周期性记账** | ✅ v1.1 已完成：数据模型（IRecurringRule/IPendingDraft）+ 规则管理页 + 启动时检测 + 首页草稿提示卡；<br>🔲 v1.2 未完成：订阅消息推送 + 云函数定时触发 + 节假日跳过选项 |
